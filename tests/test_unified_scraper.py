...
from src.data_collection.unified_scraper import UnifiedScraper

class TestUnifiedScraper(unittest.TestCase):
    def setUp(self):
        self.scraper = UnifiedScraper()

    @patch('src.data_collection.unified_scraper.requests.get')
    ...
    def test_scrape_ecommerce(self, mock_get):
        ...
        mock_get.return_value = mock_response
        products = self.scraper.scrape_ecommerce('http://test.com')
        self.assertEqual(len(products), 1)
        self.assertEqual(products[0]['name'], 'Test Product')
